...
import { User, Code, Cpu, Globe, Award, Zap, Brain, Sparkles, Layout, Layers, Box, Terminal, Github, ExternalLink } from 'lucide-react';

const Profile: React.FC = () => {
  const { settings, openApp } = useStore();
  ...
  return (
    <div className={`h-full flex flex-col md:flex-row ${isDark ? 'bg-chronos-dark text-white' : 'bg-gray-50 text-slate-800'} overflow-hidden`}>
      ...
      <div className={`w-full md:w-80 p-6 flex flex-col items-center border-b md:border-b-0 md:border-r ${isDark ? 'border-neuro-purple/20 bg-black/20' : 'border-gray-200 bg-white'}`}>
        ...
        <div className="mt-auto mb-4 w-full flex gap-2">
             <button onClick={() => openApp('mail')} className="flex-1 py-2 bg-gradient-to-r from-neuro-purple to-quantum-energy rounded-lg text-white font-bold text-sm shadow-lg hover:shadow-neuro-purple/50 transition-all flex items-center justify-center gap-2">
                 <Box size={14} /> Contact
             </button>
             <a 
                ...
                target="_blank"
                rel="noreferrer"
                className="w-12 py-2 bg-black/50 border border-white/10 rounded-lg text-white flex items-center justify-center hover:bg-white/10 transition-colors"
             >
                 <Github size={18} />
             </a>
        </div>
