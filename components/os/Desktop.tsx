...
import { RefreshCw, Image, FolderPlus, Monitor, Lock, Terminal, Github, Cpu, Calendar, Trash2 } from 'lucide-react';
...
const Desktop: React.FC<DesktopProps> = ({ apps }) => {
    ...
    // Open Files app but specifically navigating to trash is handled inside files app via props usually,
    // but here we just open the app and it will be up to user to click trash, or we could add a store action to set path.
    const openRecycleBin = () => {
        openApp('files');
        // Ideally we dispatch an action to set path to trash here, for now simpler implementation
        addNotification({ title: "Opening Trash", message: "Accessing deleted files...", appId: "files" });
    };
        ...
        >
            ...
            <div className="absolute top-4 right-4 flex flex-col gap-6 items-end p-4 z-0">
               ...
               <button 
                    ...
                    onDoubleClick={(e) => { e.stopPropagation(); openRecycleBin(); }}
                    className="flex flex-col items-center gap-2 group w-24 text-center transition-all hover:scale-105 active:scale-95"
               >
                   <div className={`w-16 h-16 rounded-2xl flex items-center justify-center border backdrop-blur-md transition-all shadow-lg group-hover:shadow-[0_0_20px_rgba(255,50,50,0.3)] ${settings.darkMode ? 'bg-white/5 border-white/10 group-hover:bg-red-900/20' : 'bg-white/40 border-white/40 group-hover:bg-white/60'}`}>
                        <Trash2 className="w-8 h-8 text-gray-400 group-hover:text-red-400 transition-colors" />
                   </div>
                   <span className={`text-[10px] font-display font-medium px-2 py-0.5 rounded backdrop-blur-md transition-colors border border-transparent ${settings.darkMode ? 'text-white bg-black/40 group-hover:text-red-300 group-hover:border-white/10' : 'text-slate-800 bg-white/40'}`}>
                       Trash
                   </span>
               </button>
            </div>
